 ...
 *   C8 半注册 `NativeModules` 不误判能力（只注册 AudioPlayer / 只注册 TTSEngine / 都不注册）。
 *
...
// ── C8 半注册 NativeModules 不误判 ───────────────────────────────────────

describe('QA · C8 半注册 NativeModules 能力判定', () => {
  const globalScope = globalThis as { NativeModules?: unknown }
  const saved = globalScope.NativeModules

  afterEach(() => {
    if (saved === undefined) {
      delete globalScope.NativeModules
    } else {
      globalScope.NativeModules = saved
    }
  })
  ...
  /** 与 `index.ts` 同构的实时合成探测（原生 TTS → Web → 不可用）。 */
  function detectRealtime(): TtsPort {
    return createNativeTts() ?? createWebTts() ?? new UnsupportedTtsPort()
  }

  it('只注册 AudioPlayer：播放可用、合成不可用（不误判「合成可用」）', () => {
    globalScope.NativeModules = { AudioPlayer: audioPlayerStub }
    const audio = createAudioPlayer()
    expect(audio).not.toBeNull()
    expect(createNativeAudioPlayer()).not.toBeNull()
    expect(createNativeTts()).toBeNull()
    const realtime = detectRealtime()
    expect(realtime.getCapability()).toBe('unsupported')

    const port = new ResolvingTtsPort(capturingSource().source, audio, realtime)
    expect(port.getCapability()).toBe('supported')
  })

  it('只注册 TTSEngine：跳过 HTTP 路径，直接实时合成', async () => {
    globalScope.NativeModules = { TTSEngine: ttsEngineStub }
    const audio = createAudioPlayer()
    expect(audio).toBeNull() // node 无 DOM Audio
    expect(createNativeTts()).not.toBeNull()

    const { source, log } = capturingSource()
    const port = new ResolvingTtsPort(source, audio, detectRealtime())
    const res = await port.speak('ねこ')
    expect(res).toEqual({ ok: true, engine: 'native' })
    expect(log).not.toContain('source.synthesize') // 不浪费流量
  })

  it('两模块都未注册：能力 unsupported，speak 不抛、返回判别联合', async () => {
    globalScope.NativeModules = {}
    expect(createAudioPlayer()).toBeNull()
    expect(createNativeTts()).toBeNull()
    expect(
      new ResolvingTtsPort(
