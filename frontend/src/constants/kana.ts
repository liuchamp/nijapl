 ...
 * **必须显式枚举**（不能用「码点比前一个假名小」之类的大小比较推断）：
 * `っ`（促音）码点比 `き` **大**，`ー`（長音）干脆不在假名区段，
 * 这两类都独立成拍，靠码点大小写必然判错 —— 所以逐项列出。
 *
 * **`ゎ` / `ヮ` 是刻意保留的**（不是想当然从 `わ` / `ワ` 推出来的）：
 * 合拗音 `くゎ` / `ぐゎ` 在语流上是**一拍**，与 `きゃ` 同类。
 * 若要改成「`ゎ` 不参与合并」，必须同时删掉这两项并说明理由 ——
 * 它们参与合并是本表的设计意图，不是漏项。
 */
