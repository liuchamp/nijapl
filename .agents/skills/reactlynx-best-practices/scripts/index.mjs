...
function collectNamedRanges(names, functionRanges) {
  return functionRanges.filter(
    (range) => void 0 !== range.name && names.has(range.name),
  )
}
...
function formatScanReport(summary) {
  ...
  if (summary.results.length > 0) {
    ...
    for (const result of summary.results)
      if (0 !== result.diagnostics.length) {
...
class ReactLynxWorkflow {
  ...
  generateFixPlan() {
    ...
    for (const result of this.context.scanResults.results) {
      ...
      for (const diagnostic of result.diagnostics) {
        const hasAutoFix =
          void 0 !== diagnostic.fixes && diagnostic.fixes.length > 0
...
export {
  analyzeBackgroundOnlyUsage,
  analyzeLifecycleUsage,
  analyzeSource,
  applyFix,
  applyFixes,
  createScanSummary,
